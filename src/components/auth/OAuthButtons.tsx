import React from 'react';
import { api } from '@/api/client';

export function GoogleIcon({ className = 'h-4 w-4' }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 18 18" aria-hidden="true">
      <path
        fill="#4285F4"
        d="M17.64 9.2c0-.64-.06-1.25-.16-1.84H9v3.48h4.84a4.14 4.14 0 0 1-1.8 2.72v2.26h2.9c1.7-1.57 2.7-3.88 2.7-6.62z"
      />
      <path
        fill="#34A853"
        d="M9 18c2.43 0 4.47-.8 5.96-2.18l-2.9-2.26c-.8.54-1.84.86-3.06.86-2.35 0-4.34-1.59-5.05-3.72H.96v2.33A9 9 0 0 0 9 18z"
      />
      <path
        fill="#FBBC05"
        d="M3.95 10.7A5.4 5.4 0 0 1 3.67 9c0-.59.1-1.16.28-1.7V4.97H.96A9 9 0 0 0 0 9c0 1.45.35 2.83.96 4.03l2.99-2.33z"
      />
      <path
        fill="#EA4335"
        d="M9 3.58c1.32 0 2.5.45 3.44 1.35l2.58-2.58C13.46.89 11.43 0 9 0A9 9 0 0 0 .96 4.97l2.99 2.33C4.66 5.17 6.65 3.58 9 3.58z"
      />
    </svg>
  );
}

export function MicrosoftIcon({ className = 'h-4 w-4' }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 18 18" aria-hidden="true">
      <rect x="0" y="0" width="8.5" height="8.5" fill="#F25022" />
      <rect x="9.5" y="0" width="8.5" height="8.5" fill="#7FBA00" />
      <rect x="0" y="9.5" width="8.5" height="8.5" fill="#00A4EF" />
      <rect x="9.5" y="9.5" width="8.5" height="8.5" fill="#FFB900" />
    </svg>
  );
}

interface OAuthButtonsProps {
  className?: string;
}

export function OAuthButtons({ className = '' }: OAuthButtonsProps) {
  return (
    <div className={`grid grid-cols-1 sm:grid-cols-2 gap-3 ${className}`}>
      <a
        href={api.googleLoginUrl()}
        className="flex items-center justify-center gap-2.5 rounded-lg border border-border bg-surface px-4 py-2.5 text-xs font-semibold text-text-main shadow-sm transition-all hover:border-primary/50 hover:bg-card active:scale-[0.98]"
        aria-label="Sign in with Google"
      >
        <GoogleIcon className="h-4 w-4 shrink-0" />
        <span>Continue with Google</span>
      </a>

      <a
        href={api.microsoftLoginUrl()}
        className="flex items-center justify-center gap-2.5 rounded-lg border border-border bg-surface px-4 py-2.5 text-xs font-semibold text-text-main shadow-sm transition-all hover:border-primary/50 hover:bg-card active:scale-[0.98]"
        aria-label="Sign in with Microsoft"
      >
        <MicrosoftIcon className="h-4 w-4 shrink-0" />
        <span>Continue with Microsoft</span>
      </a>
    </div>
  );
}

export default OAuthButtons;
